...
<script type="text/javascript">
	...
	function addRow(tableID) {
		...
		if(count == 4){
			element3.setAttribute("id","datepicker4");
		}else if(count == 5){
			element3.setAttribute("id","datepicker5");
		}
		
		cell6.appendChild(element3);
		//alert("datepicker"+count);
		count++;
...
<form method="POST" action="wsals" name='leave'>
<h3 align="center" class="bars">Leave Application</h3><br>
  <table align="center" style="width: auto;">
    ...
    <tr>
      ...
      <td><input type='number' name='noOfDays' step='0.5' min='0' placeholder="must be numbers" class="form-control" required="required"/></td>
    </tr>
    <tr>
      <td><b>Date:</b></td>
      <td><input type='text' name='fromDate' placeholder="from" id="datepicker1"  required="required" /><input type='text' name='toDate' placeholder="to" id="datepicker2"  required="required"/></td>
	</tr>
	<tr>
      <td><b>Purpose:</b></td>
      <td>
	  
	  <textarea cols=35 rows=1 name="purpose" placeholder="no special characters please" required="required" onblur="specialcharecter()" class="form-control"></textarea>
...
<table  width="100%"  align="center" border="1">
	...
	<%
	   ...
	   rs=s.executeQuery("select * from leaves  where applicant_id ='"+uid+"' order by id desc");
	%>
	<tr>
		<th>Leave Type</th>
		<th>Num of Days</th>
	...
	%> 
			...
			<TR>
				...
				<TD width="15%"><%=rs.getString("leave_type")%> </TD>
				<td width="10%"> <%=rs.getString("numdays")%></td>
				<TD width="20%"><%=rs.getString("fromDate")%> To <%=rs.getString("toDate")%>  </TD>
				<TD width="15%"> <%=rs.getString("purpose")%></TD>
				<TD width="10%"> <%=rs.getString("hodstatus")%></TD>
				<TD width="15%"> <%=rs.getString("principalstatus")%></TD>
					<td width="10%"><%=rs.getString("rejectreason")%></td>
		
				<%if(rs.getString("hodstatus").equals("Pending")) {%>
				<TD width="5%" style="border-color: white;"> <a href="leaveApplyEdit.jsp?uid=<%=uid%>&lId=<%=rs.getString("id")%>">Edit</a>&nbsp;&nbsp;<a href="deleteLeave.jsp?uid=<%=uid%>&lId=<%=rs.getString("id")%>">Delete</a></TD>
				<%
	 				 }
	  			%>
	  		</tr>
	  ...
	  <%
		rs1=s.executeQuery("select * from  adjust  where applicant_id ='"+uid+"' order by adj_date desc");
		while(rs1.next()){
		%>
			<tr>
				<TD width="10%"> <%=rs1.getString("sem")%></TD>
				<td width="10%"><%=rs1.getString("section")%></td>
				<TD width="10%"> <%=rs1.getString("adj_date")%></TD>
				<TD width="5%"><%=rs1.getString("period")%> - <%=rs1.getString("session")%> </TD>
				<td width="10%"><%=rs1.getString("adj_with_id")%>	</td>
				<td width="10%"><%=rs1.getString("fadjuststatus")%>	</td>
