...
if(type.equals("issue")){
	...
	rs1=s1.executeQuery("select no_of_books from book_issue where  user='"+user+"' and status='issue'");
	if(rs1.next()){
		no_of_books=rs1.getInt("no_of_books");
	}
	...
	if(no_of_books<=6){
		...
		if(rs.next()){
			if(rs.getString("status").contains("available")){
				...
				out.println("Successfully Isseud book "+accno); 
			}else{
